...
(function rememberProgress() {
  var stepsCompleted = parseInt(localStorage.getItem("stepsCompleted"));

  //If first step was completed, show the date selected panel
  if (stepsCompleted >= 1) {
    document.getElementById("date-selection").style.display = "none";
    var dateString = localStorage.getItem("dateString"); //changing the yyyy-mm-dd format to mm/dd/yyyy
  ...
  //If second step was completed, show payment complete visual
  if (stepsCompleted >= 2) {
    document.getElementById("payment-info").style.display = "none";
    document.getElementById("making-payment").style.display = "none";
    document.getElementById("payment-confirmation").style.display = "inline";
  }
})();
...
function setOrientationDate() {
  ...
  //unhide next and increment stepsCompleted in localStorage
...
function paymentConfirmation() {
  ...
  //unhide next and increment stepsCompleted and nextCounter in localStorage
  document.querySelector("#next").style.display = "inline";
...
function agreementStatusCheck() {
  var agreementCheckbox = document.querySelector("#coc-agreement");

  //If the checkbox is checked, display next and increment stepsCompleted in LS
  if (agreementCheckbox.checked == true) {
    document.querySelector("#next").style.display = "inline";
    localStorage.setItem("stepsCompleted", 3);
  }

  //Else undo those changes
  else {
...
function setupComplete() {
  auth.onAuthStateChanged((user) => {
    ...
    //Remove nextCounter, stepsCompleted and dateString in LS
    localStorage.removeItem("nextCounter");
...
function nextControl() {
  //Increment and update nextCounter
  var nextCounter = parseInt(localStorage.getItem("nextCounter"));
  nextCounter++;
  localStorage.setItem("nextCounter", nextCounter);

  //If nextCounter is greater than steps completed, hide it
  var stepsCompleted = parseInt(localStorage.getItem("stepsCompleted"));
  if (nextCounter > stepsCompleted)
    document.querySelector("#next").style.display = "none";
...
function prevControl() {
  ...
  //Decrement and updatenextCounter 
  var nextCounter = parseInt(localStorage.getItem("nextCounter"));
  nextCounter--;
  localStorage.setItem("nextCounter", nextCounter);

  //If next counter is equal to 0, hide it
  if (nextCounter == 0)
...
(function settingControlValues() {
  ...
  var stepsCompleted = localStorage.getItem("stepsCompleted");
  if(!stepsCompleted) //Checking if it doesn't exist
    localStorage.setItem("stepsCompleted", 0);
  
  localStorage.setItem("nextCounter", 0);
